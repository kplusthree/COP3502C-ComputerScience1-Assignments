...
if [ ! -f Orcheeseo.c ]; then
	echo ""
	echo " Error: You must place Orcheeseo.c in this directory before"
	echo "        we can proceed. Aborting test script."
	echo ""
	exit
elif [ ! -f Orcheeseo.h ]; then
	echo ""
	echo " Error: You must place Orcheeseo.h in this directory before"
	echo "        we can proceed. Aborting test script."
	echo ""
	exit
...
grep -s -q "^#include \"Orcheeseo\.h\"" Orcheeseo.c
grep_val=$?

if [[ $grep_val != 0 ]]; then
	echo ""
	echo "  Whoa, buddy! Your Orcheeseo.c file does not appear to have a proper"
	echo "  #include statement for Orcheeseo.h. Please read Section 2 of the"
	echo "  assignment PDF before proceeding."
	echo ""
	exit
fi

# Save a backup version of Orcheeseo.h to be restored after running this
# script, so that the #define main line ends up back how you had it.
cp Orcheeseo.h Orcheeseo_backup.h
...
run_test_case()
{
	...
	# Ensure that Orcheeseo.h is in normal test case mode (not unit test mode).
	perl -p -i -e 's/^#define main/\/\/#define main/' Orcheeseo.h

	# Attempt to compile.
	gcc Orcheeseo.c 2> /dev/null
	compile_val=$?

	# Immediately restore header file in case the user terminates the script
	# before we can move on to the next test case.
	cp Orcheeseo_backup.h Orcheeseo.h
...
run_unit_test()
{
	...
	# Ensure that Orcheeseo.h is in unit test case mode.
	perl -p -i -e 's/^\/\/#define main/#define main/' Orcheeseo.h

	# Attempt to compile.
	gcc Orcheeseo.c UnitTestLauncher.c $testcase_file 2> /dev/null
	compile_val=$?
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " Orcheeseo.c | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" Orcheeseo.c | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" Orcheeseo.c | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" Orcheeseo.c -n"
		echo "     grep \"\$(printf '^\t')\" Orcheeseo.c -n"
...
if [ -f Orcheeseo_backup.h ]; then
	mv Orcheeseo_backup.h Orcheeseo.h
fi
